...
class LibroStockDisponibleTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.autor = Autor.objects.create(nombre="Julio", apellido="Verne")

    def test_disponible_se_calcula_con_stock(self):
        libro = Libro.objects.create(titulo="X", autor=self.autor, stock=0, disponible=True)
...
class PrestamoDiasRetrasoTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.autor = Autor.objects.create(nombre="Isaac", apellido="Asimov")
        cls.usuario = User.objects.create_user("u", password="12345")
        cls.libro = Libro.objects.create(titulo="Fundación", autor=cls.autor, stock=1)

    def test_dias_retraso_con_devolucion(self):
        ...
        self.assertEqual(prestamo.dias_retraso, 5)
        self.assertGreater(prestamo.multa_retraso, 0)